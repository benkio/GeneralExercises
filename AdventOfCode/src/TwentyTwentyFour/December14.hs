...
findByAdjacentRobots adjacents nMax bSize rs = go 1 (moveRobots bSize rs)
  where
    go !n rs' =
        if atLeastNAdjacentRobotsByDirection adjacents bSize rs' || n >= nMax
            then n
            else
                go
                    ( -- traceShowId
...
december14Solution2Print =
    ( \i ->
        -- putStrLn (printRobots bathroomSize (evolveToT (solution2 i) bathroomSize i))
        return ()
    )
        =<< input
