module TwentyFifteen.FifteenthDecember where
...
ingredientTablespoon Ingredient { capacity = ic
                                , durability = id'
                                , flavor = if'
                                , texture = it
                                , calories = ical
  ...
  (maximum .
   fmap
     ((\l ->
      ...
      fmap sum . transpose . fmap (init . uncurry ingredientTablespoon) . zip is)) $
