module TwentyFifteen.FourteenthDecember where
...
fourteenthDecemberSolution1 = solution1 2503 <$> input

rendeerRace2 :: [Rendeer] -> [[Int]]
rendeerRace2 rs =
  (scanl calculateLeader (fmap (const 0) rs) .
   transpose . fmap (tail . rendeerRace1))
    rs
...
fourteenthDecemberSolution2 = solution2 2503 <$> input
