...
import Data.Map (Map)
import qualified Data.Map as M (insert, empty, elems)
import Data.Tree (Tree)
import qualified Data.Tree as T (unfoldForest, foldTree)
import Debug.Trace
import Text.Read
  ...
  T.unfoldForest
    ( \(d, w, z) ->
        let b = bs !! d
            ...
            z' = reg_z st
         in ((w, reg_z st), [(d + 1, w', z') | w' <- [9,8..1], (d + 1) < length bs, z' < zLimit])
    )
    [(0, w, 0) | w <- [9,8..1]]

modelNumbers :: Tree (Int, Int) -> Maybe String
modelNumbers = T.foldTree treeToNum
  where treeToNum (w, z) [] = if z == 0 then Just (traceShow z (show w)) else Nothing
        treeToNum (w, _) xs = let vals = (fmap (\s -> read s :: Int) . catMaybes) xs
...
relevantInputValues i (b:bs) = (i, unsafeGetValue (b !! 4), unsafeGetValue (b !! 14)):relevantInputValues (i+1) bs
  where unsafeGetValue (Instruction _ _ (Right v)) = v
...
twentyFourthDecemberSolution1 = do
  ...
  let rv = relevantInputValues 0 i
      ...
      s = ((\x -> read x :: Int) . concat . fmap show . M.elems) ms
  return s
