...
-------------------------------------------------------------------------------
module TwentyTwenty.December19 where
...
ruleToValidString (CompositeRule _ rs1 rs2) rules =
    foldl1 stringProduct (ruleIdToString <$> rs1)
        `union` foldl1 stringProduct (ruleIdToString <$> rs2)
  where
    ruleIdToString :: Int -> Set String
    ruleIdToString = (`ruleToValidString` rules) . getRule rules
...
december19Solution2 = solition2 <$> input
