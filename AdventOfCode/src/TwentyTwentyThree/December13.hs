module TwentyTwentyThree.December13 where
...
findHReflection f = listToMaybe $ mapMaybe (checkMirrorPair f pairs) mirrorPairs
  where
    pairs = zip f (tail f)
    mirrorPairs = filter (uncurry (==)) pairs
...
checkMirrorPair f pairs mirrorPair =
    let topReflection = ((++ [fst mirrorPair]) . fmap fst . takeWhile (/= mirrorPair)) pairs
        ...
        isReflection = all (uncurry (==)) $ zip (reverse topReflection) bottomReflection
...
findSmudgeIndex field f = (fmap f . find checkIndex) [1 .. length field - 1]
  where
    ...
    checkIndex i = ((== 1) . sum) $ (\(l, l') -> (length . filter not . fmap (uncurry (==))) (zip l l')) <$> zip (reverse (topReflection i)) (bottomReflection i)
