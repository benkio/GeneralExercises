...
spec = describe "validateInput" $ do
    it "should return a Call if the input is correct" $
        property $
            forAll (generateCallLog True) (isJust . validateInput)
    it "should return Nothing if the input is incorrect" $
        property $
            forAll (generateCallLog False) (isNothing . validateInput)
    describe "groupByCostumer" $ do
        it "should contain all the input calls as elements" $
            property $
                forAll
                    calls
                    (\input -> (length . concat . elems . groupByCostumer) input == length input)
        it "should contain all the input costumers as keys" $
            property $
                forAll
                    calls
