...
spec = describe "parseCallsLog" $ do
    it "should return a CallLog when a valid input is provided" $
        property $
            forAll
                (generateCallLog True)
                ( \validInput -> do
    ...
    it "should return a list of CallLog of the same size of the input list" $
        property $
            forAll
                (listOf (generateCallLog True))
                ( \listValidInput -> do
    ...
    describe "parseCall" $ do
        it "should return a Call when a valid input is provided" $
            property $
                forAll
                    (generateCallLog True)
                    ( \validInput -> do
                        ...
                        ( ( \d ->
                                ((printf "%02s") . show . TestHours) (durationHours d)
                                    ++ ":"
                                    ++ ((printf "%02s") . show . TestMinutes) (durationMinutes d)
                                    ++ ":"
                                    ++ ((printf "%02s") . show . TestSeconds) (durationSeconds d)
                            )
                                . duration
                                . head
                            )
                            result
                            `shouldBe` inputDuration
                    )
    it "should return a list of CallLog of the same size of the input list" $
        property $
            forAll
    ...
    it "should return an empty list if the input is invalid" $
        property $
            forAll
                (generateCallLog False)
                ( \invalidInput -> do
                    writeFile "test.log" invalidInput
                    result <- parseCalls "test.log"
                    null result `shouldBe` True
                )
    it "should return an empty list if the input is invalid (List)" $
        property $
            forAll
                (listOf (generateCallLog False))
                ( \invalidInput -> do
                    let input = (concat . intersperse "\n") invalidInput
                    writeFile "test.log" input
                    result <- parseCalls "test.log"
