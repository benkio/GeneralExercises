module SecondDecember(solution1, solution2) where
...
lineToPassword l =
  let w = words l
      ...
      maxAppearence  = ((\x -> read x :: Int) . tail . dropWhile ('-' /=)) (head w)
...
validPasswords validation = foldr (\p v -> if validation p then v+1 else v) 0

isValidPasswordOld :: Password -> Bool
isValidPasswordOld (Password (Policy min' max' char) password)
 | ( (\count -> count >= min' && count <= max') . length . filter (char ==)) password = True
 | otherwise = False
