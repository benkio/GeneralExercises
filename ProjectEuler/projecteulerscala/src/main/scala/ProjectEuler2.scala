...
object ProjectEuler2:
  ...
  def es13: String =
    ...
    input.linesIterator.map(s => BigInt(s.trim)).sum.toString.take(10)
  ...
  def es14: Long =
    ...
    def chainLength(
      ...
      value match {
        ...
        case v if v % 2 == 0 => chainLength(v / 2, v :: computed, visited)
...
04 62 98 27 23 09 70 98 73 93 38 53 60 04 23""".linesIterator
        .map(_.split(" ").padTo(15, "0").map(_.toInt))
        .toList
    val result = input.foldRight(List.fill(15)(0)) { case (prevLine, lastLine) =>
      lastLine.sliding(2).map(_.max).padTo(15, 0).zip(prevLine).map { case (x, y) => x + y }.toList
    }
    result.head
