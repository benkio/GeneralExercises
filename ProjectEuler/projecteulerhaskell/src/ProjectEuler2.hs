...
import Data.List (find, foldl', isPrefixOf, isSuffixOf, transpose, union)
...
es14 =
    fst $
        foldr
            ( \x acc ->
                let currentCollatz = collatzSequence x
                 in if currentCollatz > snd acc
                        then (x, currentCollatz)
                        else acc
            )
            (1, 1)
            [1 .. 1000000]
...
threeDigitStringCount n =
    let
     ...
     in
        hundredsStringCount ++ twoDigitCount
  where
    oneHundredCheck :: (Int, Int) -> String
    oneHundredCheck (h, h')
...
fourDigitStringCount n =
    let
     ...
     in
        thousendStringCount ++ twoDigitCount
  where
    oneThousandCheck :: (Int, Int) -> String
